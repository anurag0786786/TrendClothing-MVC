            ...
            { "data": "name", "render": data => `<span style="font-weight:600;">${data}</span>` },
            {
                ...
                "render": data =>
                    `<div style="display:flex;gap:8px;">
                        <a href="/Admin/Size/Upsert/${data}" class="tc-admin-btn-edit">
                        ...
                        <button class="tc-admin-btn-delete" onclick="Delete('/Admin/Size/Delete/${data}')">
...
function Delete(url) {
    swal({ title: "Delete size?", icon: "warning", buttons: ["Cancel", "Delete"], dangerMode: true })
        .then(ok => {
            if (ok) {
                $.ajax({
